<template>
	<div class="Newest">
		...
		<van-panel class="list" >
			...
			<div>
				<van-row class="musicCart" ref="van" v-for="(item,index) in music" :key="index" type="flex" justify="space-between" align="center">
...
<script>
	...
	export default {
		...
		mounted(){
			this.axios({
				method: 'POST',
				url: 'https://api.itooi.cn/music/netease/songList?key=579621905&id=134953476',
			}).then(result => {
				//console.log('result ==> ', result);
				this.data = result.data.data.songs
				var datamusiccart = JSON.stringify(this.data)
				localStorage.setItem('Newestmusic',datamusiccart)
			})
			var timers = [];
			window.onscroll = () => {
				//滚屏的距离
		...
		created() {
			...
			var data = localStorage.getItem('Newestmusic')
